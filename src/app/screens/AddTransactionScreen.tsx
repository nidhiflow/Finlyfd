  ...
  return (
    <div className="relative" style={{ background: "var(--bg-deep)", minHeight: "100vh" }}>
      ...
      <div className="relative z-10 pb-[80px]">
        ...
        {/* ── Type Toggle ── */}
        <div className="flex gap-2 mx-4 mb-2.5 bg-[var(--surface)] rounded-[10px] p-1 mt-2">
          {[
            ...
            return (
              <motion.button
                ...
                whileTap={{ scale: 0.97 }}
                className="flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded-[8px] transition-colors"
                style={{
                  fontSize: 12.5,
                  fontWeight: 500,
                ...
                <t.icon className="w-[13px] h-[13px]" strokeWidth={1.75} />
                {t.label}
              </motion.button>
            )
          })}
        ...
        {/* ── Amount Card ── */}
        <div className="mx-4 mb-2.5 bg-[var(--surface)] rounded-[14px] p-3.5 relative overflow-hidden">
          ...
          <div className="flex justify-between items-center mb-1.5 relative z-10">
            ...
            <motion.button whileTap={{ scale: 0.9 }} onClick={() => setShowCalc(true)}
              className="flex items-center gap-1 text-[11px] font-medium px-2 py-1 rounded-[16px]"
              style={{
                 color: typeAccent,
                 background: `${typeAccent}25`
              }}>
              <Calculator className="w-[12px] h-[12px]" style={{ color: typeAccent }} />
              Calc
            </motion.button>
          </div>
          <div className="flex items-baseline gap-1.5 font-fraunces font-medium text-[32px] relative z-10" style={{ color: typeAccent }}>
        ...
        {/* ── Note ── */}
        <div className="mx-4 mb-2.5 bg-[var(--surface)] rounded-[14px] px-3.5 py-2.5 relative z-40">
           <div className="flex items-center gap-2">
             <p className="text-[10px] tracking-[0.07em] uppercase text-[var(--ink-muted)] m-0 w-[40px]">Note</p>
             <input
                value={note}
                ...
                className="flex-1 bg-transparent border-none text-[var(--ink)] font-inter text-[13px] outline-none placeholder:text-[var(--ink-faint)]"
             />
           </div>
           <AnimatePresence>
               ...
               <motion.div
                 ...
                 {filteredNotes.map((s, i) => (
                   <div key={i} 
                     ...
                     className="px-3.5 py-2.5 border-b border-[var(--divider)] last:border-b-0 active:bg-ink/5 cursor-pointer text-[var(--ink)] transition-colors"
                     style={{ fontSize: 13 }}>
        ...
        {txType !== "transfer" && (
          <div className="mb-2.5">
            <div className="flex items-center justify-between mx-4 mb-1.5">
               <p className="text-[10px] tracking-[0.07em] uppercase text-[var(--ink-muted)] m-0">Category</p>
               {catId && (
                  <button onClick={resetCat}
                    className="flex items-center gap-1 px-2 py-0.5 rounded-[10px]"
              ...
              {cats.map(c => {
                ...
                return (
                  <motion.button key={c.id}
                    ...
                    className="bg-[var(--surface)] rounded-[12px] p-2 flex flex-col items-center gap-1.5 flex-shrink-0 w-[68px]"
                    style={{
                    ...
                    <div className="w-[28px] h-[28px] rounded-full flex items-center justify-center"
                      ...
                      {c.icon ? <c.icon className="w-3.5 h-3.5" /> : <span style={{ fontSize: 14 }}>{c.emoji}</span>}
                    </div>
                    <span className="text-[9.5px] text-center leading-[1.2]" style={{ color: isSel ? "var(--ink)" : "var(--ink-muted)" }}>
            ...
            <AnimatePresence>
              {catId && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }}
                  exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.22 }}
                  className="px-4 mt-1 overflow-hidden">
                    ...
                    style={{
                      ...
                      <div className="flex-1 ml-1">
                        <div className="flex items-baseline gap-2">
                           <p className="text-ink font-semibold m-0" style={{ fontSize: 11.5 }}>{selectedCat?.name}</p>
                           {subId && <span className="text-ink/60 m-0" style={{ fontSize: 10.5 }}>→ {selectedSub?.name}</span>}
                        </div>
                      </div>
        ...
        <div className="grid grid-cols-2 gap-2.5 mx-4 mb-3">
           ...
           <motion.button whileTap={{ scale: 0.98 }} onClick={() => setShowAccSheet(true)}
              className="rounded-[12px] p-2.5 flex items-center gap-2 text-left"
              style={{
                 background: `${typeAccent}25`,
                 border: errors.acc ? "1px solid #EF4444" : "none"
              }}>
              <div className="w-[28px] h-[28px] rounded-full flex items-center justify-center flex-shrink-0" style={{ background: "rgba(0,0,0,0.2)", color: typeAccent }}>
                 {(() => {
                   const AccIcon = getAccountIcon(selectedAcc.type);
                   return <AccIcon className="w-[13px] h-[13px]" />;
                 })()}
              </div>
              <div className="flex-1 overflow-hidden">
                 <p className="m-0 text-[11px] font-medium text-[var(--ink)] truncate">{selectedAcc.name}</p>
                 <span className="text-[11px] font-semibold tabular-nums" style={{ color: typeAccent }}>₹{Math.abs(selectedAcc.balance).toLocaleString("en-IN")}</span>
              </div>
           </motion.button>

           {/* Date & Time */}
           <motion.button whileTap={{ scale: 0.98 }} onClick={() => setShowDate(true)}
              className="bg-[var(--surface)] rounded-[12px] p-2.5 flex items-center gap-2">
              <div className="w-[28px] h-[28px] rounded-[8px] flex items-center justify-center flex-shrink-0" style={{ background: "var(--orange-bg)", color: "var(--orange)" }}>
                 <Calendar className="w-[13px] h-[13px]" />
              </div>
              <div className="flex-1 overflow-hidden text-left">
                 <p className="m-0 text-[11px] text-[var(--ink-muted)]">Date & time</p>
                 <span className="text-[11px] font-medium text-[var(--ink)] truncate block">{fmtDate(date)}</span>
              </div>
           </motion.button>
        </div>
        ...
        {txType === "transfer" && (
          <motion.button whileTap={{ scale: 0.98 }} onClick={() => setShowToAcc(true)}
             className="mx-4 mb-3 rounded-[12px] p-2.5 flex items-center gap-2 text-left w-[calc(100%-32px)]"
             style={{
             ...
             {selectedTo ? (
                <>
                   <div className="w-[28px] h-[28px] rounded-full flex items-center justify-center flex-shrink-0" style={{ background: "rgba(0,0,0,0.2)", color: selectedTo.color }}>
                      {(() => {
                        const ToAccIcon = getAccountIcon(selectedTo.type);
                        return <ToAccIcon className="w-[13px] h-[13px]" />;
                      })()}
                   </div>
                   <div className="flex-1 overflow-hidden">
                      <p className="m-0 text-[11px] font-medium text-[var(--ink)] truncate">To: {selectedTo.name}</p>
                      <span className="text-[11px] font-semibold tabular-nums" style={{ color: selectedTo.color }}>₹{Math.abs(selectedTo.balance).toLocaleString("en-IN")}</span>
                   </div>
                   <ChevronDown className="w-[13px] h-[13px] ml-2" style={{ color: selectedTo.color }} strokeWidth={1.75} />
                </>
             ) : (
                <>
                   <div className="w-[28px] h-[28px] rounded-full flex items-center justify-center flex-shrink-0" style={{ background: "var(--divider)", color: "var(--ink-muted)" }}>
                      <Plus className="w-[13px] h-[13px]" />
                   </div>
                   <p className="flex-1 m-0 text-[12px] font-medium text-[var(--ink-muted)]">Select destination account</p>
                </>
             )}
          </motion.button>
        )}
        ...
        <div className="flex gap-2 mx-4 mb-4">
           ...
           <motion.button
              ...
              disabled={aiScanning}
              className="flex-[2] rounded-[12px] py-2.5 flex items-center justify-center gap-1.5 text-[12px] font-semibold text-[#241B0A]"
              style={{ background: "linear-gradient(135deg, var(--gold), #E8B86B)" }}>
              {aiScanning ? (
                <motion.div animate={{ rotate: 360 }} transition={{ duration: 1, repeat: Infinity, ease: "linear" }} className="w-3.5 h-3.5 rounded-full border-2 border-[#241B0A]/30 border-t-[#241B0A]" />
              ) : (
                <>
                   <Sparkles className="w-[13px] h-[13px] stroke-[#241B0A]" />
                   Scan Receipt
                </>
              )}
           </motion.button>
           ...
           <div className="flex gap-1.5 flex-1">
             <motion.button whileTap={{ scale: 0.96 }}
                className="flex-1 bg-[var(--surface)] rounded-[12px] flex items-center justify-center text-[var(--ink-muted)]">
           ...
           <motion.button whileTap={{ scale: 0.96 }} onClick={() => setRecurring(!recurring)}
              className="px-3 bg-[var(--surface)] rounded-[12px] flex items-center justify-center transition-colors"
        ...
        <AnimatePresence>
          {recurring && (
            <motion.div
              ...
              <div className="p-3 rounded-[12px]" style={{ background: "var(--surface)", border: `1px solid ${typeAccent}30` }}>
                <p className="text-ink/60 font-semibold mb-2" style={{ fontSize: 10, letterSpacing: "0.5px" }}>RECURRING SETTINGS</p>
                <div className="flex flex-col gap-2">
                  <div className="flex items-center justify-between">
                    <span className="text-ink" style={{ fontSize: 12 }}>Frequency</span>
                    <select
                      ...
                      style={{ fontSize: 12, color: typeAccent }}>
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-ink" style={{ fontSize: 12 }}>Ends</span>
                    <select
                  ...
                  {recurEndType === "date" && (
                    <button onClick={() => setShowRecurEndDate(true)}
                      className="w-full text-right mt-1" style={{ fontSize: 11, color: "var(--ink-muted)" }}>
        ...
        <div className="mx-4 mt-1">
           <motion.button
              whileTap={{ scale: 0.97 }}
              onClick={handleSave}
              className="w-full rounded-[14px] py-3.5 flex items-center justify-center gap-1.5 text-[14px] font-semibold"
              style={{ background: typeAccent, color: "#241310" }}>
              <Check className="w-[14px] h-[14px]" style={{ stroke: "#241310" }} strokeWidth={2.5} />
              Save Transaction
           </motion.button>
        </div>
